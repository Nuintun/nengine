...
function SendStream(req, path, options){
    ...
    // Root
    this._root = isType(options.root, 'string')
        ? resolve(options.root)
        : root;

    this._root = formatPath(this._root);
    ...
    // Dotfiles
    this._dotFiles = isType(options.dotFiles, 'string')
        ? options.dotFiles
        : 'ignore';
    ...
    // Extensions
    this._extensions = normalizeList(options.extensions);

    // Clean options
    delete options.extensions;

    // Default document
    this._index = isType(options.index, 'array') || isType(options.index, 'string')
    ...
    this._maxAge = isType(maxAge, 'number')
    ...
    // TODO test
    this._dirRedirect = false;

    // Clean options
    delete options.dirRedirect;
...
SendStream.prototype.dirRedirect = function (path){
    ...
    if (listenerCount(this, 'directory') !== 0) {
        return this.emit('directory', path);
    }
...
SendStream.prototype.send = function (path, stat){
    ...
    if (ranges) {
        ...
        if (-2 !== ranges && ranges.length === 1) {
            ...
            this._range[0] = offset + ranges[0].start;
            this._range[1] = offset + ranges[0].end;
            ...
            len = this._range[1] - this._range[0] + 1;
        }
    }
    ...
    function next(err){
        ...
        if (i >= length) {
            if (listenerCount(self, 'directory') !== 0) {
                return self.emit('directory', path, dirStat);
            }
