...
function SendStream(requset, response, options){
    ...
    this.maxAge = isType(maxAge, 'number') && maxAge >= 0
...
SendStream.prototype.pipe = function (){
    ...
    if (containsDotFile(path)) {
        ...
        // Debug infomation
        debugResponse('Dot file: %s %s', path, dotFile);
...
SendStream.prototype.stream = function (path, options){
    ...
    // Emit stream event
    listenerCount(this, 'stream') > 0 && this.emit('stream', stream);
...
SendStream.prototype.setHeader = function setHeader(path, stat){
    ...
    if (this.lastModified && !res.getHeader('Last-Modified')) {
        ...
        // Debug infomation
        debugResponse('Modified: %s', modified);
