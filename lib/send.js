...
SendStream.prototype.send = function (path, stat){
  ...
  // Set content-type
  contentType = this.setContentType(path);
      ...
      if (isType(ranges, 'array')) {
        ranges.forEach(function (range, i){
          ...
          if (ranges.length > 1) {
            ...
            header.push(
              'Content-Type: ',
              contentType.type + (contentType.charset ? '; charset=' + contentType.charset : '')
            );
            header.push('Content-Range: ', start + '-' + end + '/' + stat.size);
          ...
          self.ranges.push({
            ...
            size: range.end - range.start + header.length + 1
          });
        });
        ...
        if (ranges.length === 1) {
          ...
          // Debug infomation
          debugResponse('Range: [%s, %s]', start, end);

          // Debug infomation
          debugResponse('Content-Range: bytes %s-%s/%s', start, end, length);
 ...
 * @api private
 */
SendStream.prototype.stream = function (path, stat){
  var header,
    self = this,
    res = self.response,
  ...
  // Listen write stream drain event
