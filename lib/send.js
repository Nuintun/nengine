...
function Send(root, options){
  ...
  // Root
  root = isType(root, 'string')
    ? resolve(root)
    : cwd;

  root = httpPath(root + sep);
  ...
  options.dotFiles = isType(options.dotFiles, 'string')
    ? options.dotFiles.toLowerCase()
    : 'ignore';

  // Extensions
  options.extensions = normalizeList(options.extensions);

  // Default document
  options.index = isType(options.index, 'array') || isType(options.index, 'string')
...
SendStream.prototype.parseRange = function (stat){
  ...
  if (ranges) {
    ...
    if (rangeFresh) {
      ...
      if (isType(ranges, 'array') && ranges.type === 'bytes') {
        // Content-Range
        res.statusCode = 206;
        ...
        if (ranges.length > 1) {
          ...
          debugResponse('Content-Type Reset: multipart/byteranges; boundary="<%s>"', boundary);
...
SendStream.prototype.stream = function (path){
  ...
  function concatRange(){
    ...
    // Push boundary
    if (range.boundary && !transform.write(range.boundary)) {
      // Pause stream
      stream.pause();
      // Bind once drain event
      transform.once('drain', function (){
        stream.resume();
      });
    }
