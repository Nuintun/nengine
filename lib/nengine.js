...
function httpError(response, send, error, next){
  ...
  var errorpage = self.options.status[status];
  ...
  run: function (){
    if (cluster.isMaster) {
      ...
      httpServer.on('listening', function (){
        // self.logger.info('Server runing at port: ' + options.port);
        // Logger
        process.send({
          event: 'listening',
          method: 'logger',
          type: 'info',
          message: 'Server runing at port: ' + options.port
        });
      });

      // Error
      httpServer.on('error', function (error){
        // self.logger.error('Server failed to start: ' + error.message);
        // Logger
        process.send({
          event: 'error',
          method: 'logger',
