...
function fileSend(requset, response){
  ...
  send.on('error', function (error, next){
    httpError.call(self, response, error, next);
  });
 ...
 * @param error
 */
function httpError(response, send, error){
  var self = this;
  var url = send.url;
    ...
    if (typeof errorpage === 'string') {
      fs.exists(path.join(send.root, errorpage), function (exists){
        if (exists) {
          ...
          defaultStatus.call(self, send, error);
        }
      });
    } else {
      // Default error page
 ...
 * @param next
 */
function viewFolder(send, response, fpath, next){
  var self = this;
  ...
  fs.readdir(fpath, function (error, files){
    // Response
    response.write(self.res.html['folder'](dirname, files, fpath));
