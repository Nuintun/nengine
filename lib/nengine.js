  ...
  if (options.verbose) {
    ...
    send.on('headers', function (headers){
      process.send({
        ...
        + '\r\nURL      : ' + colors.green.bold(send.url)
        + '\r\nPATH     : ' + colors.cyan.bold(send.path)
        + '\r\nROOT     : ' + colors.cyan.bold(send.root)
        + '\r\nREALPATH : ' + colors.cyan.bold(send.realpath)
        + '\r\nSTATUS   : ' + colors.magenta.bold(send.statusCode)
        + '\r\nHEADERS  : ' + colors.magenta.bold(JSON.stringify(headers, null, 2))
      });
    });
  }
...
NengineServer.prototype = {
  run: function (){
    ...
    if (cluster.isMaster) {
      ...
      // Get logger
      logger = log4js.getLogger('Nengine-Server');
